...
is a small, open way to make those claims testable. Version 0.4.0 contains
18 deterministic scenarios: 17 baseline/candidate workbook pairs and one
...
Version 0.4.0 includes a deterministic, one-row-per-case `manifest.jsonl`
...
It also upgrades the truth contract to schema version 3 with an Excel Table
...
also adds a direct-to-`INDIRECT` change and a scoreable requirement to disclose
the static-dependency boundary that text-driven references introduce.

Version 0.4.0 also upgrades the tool-neutral normalized observation protocol.
...
structured-reference text, an introduced `INDIRECT` reference, structural
formula rewrites, and a cross-workbook dependency.
...
so regeneration is byte-for-byte reproducible. Version 0.4.0 passed 23 tests
...
making its report schema normative. FormulaFence 0.219.0 recovered all 19
currently mappable facts, the one scoreable dynamic-reference coverage
declaration, and five targeted lint rules. Its normalized export reports those
facts without inventing review decisions, so policy agreement remains
explicitly unset. The structural rewrite is intentionally left unmapped: it
documents intent, but does not pretend that a small fixture proves generic
...
the [v0.4.0 release](https://github.com/SybilGambleyyu/workbook-change-benchmark/releases/tag/v0.4.0),
